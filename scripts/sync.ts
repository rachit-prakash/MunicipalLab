...
import { analyzeMessage, classifyMessageToFoldersSync, generateSummary } from "@/lib/analysis";
import {
...
export async function syncAllAccountsForUser(userId: string): Promise<void> {
  ...
  console.log(`Syncing ${accountsResult.rows.length} account(s) for user ${userId}`);
...
export async function runIncrementalSync(
  ...
  const accountInfo = await withTenant(tenantId, async (client) => {
    ...
    if (accountId) {
      result = await client.query<{
        ...
        `SELECT id, history_id, email FROM gmail_accounts WHERE user_id = $1 AND id = $2`,
        [userId, accountId]
      );
    } else {
      result = await client.query<{
        ...
        `SELECT id, history_id, email FROM gmail_accounts WHERE user_id = $1 LIMIT 1`,
        [userId]
      );
    }
  ...
  if (!accountInfo.history_id) {
    await bootstrapSync(tenantId, userId, accessToken, accountInfo.email, accountInfo.id);
  } else {
  ...
  await withTenant(tenantId, async (client) => {
    const whereClause = accountId ? `WHERE id = $1` : `WHERE user_id = $1`;
    const params = accountId ? [accountId] : [userId];
    await client.query(
      `UPDATE gmail_accounts SET last_sync_at = NOW() ${whereClause}`,
      params
    );
  });
}
...
async function bootstrapSync(
  ...
  if (profileData.historyId) {
    await withTenant(tenantId, async (client) => {
      await client.query(
        `UPDATE gmail_accounts SET history_id = $1 WHERE id = $2`,
        [profileData.historyId, accountId]
      );
    });
  }
}
  ...
  do {
    ...
    try {
      ...
      if (message.includes("status 410") || message.includes("historyId")) {
        ...
        await bootstrapSync(tenantId, userId, accessToken, userEmail, accountId);
        return;
      }
      throw error;
    }
...
async function fetchAndUpsertMessage(
  ...
  const upsertResult = await withTenant(tenantId, async (client) => {
    ...
    if (hasFolders.rows.length > 0) {
      ...
      threadResult = await client.query<{ id: string }>(
        `INSERT INTO threads (
            tenant_id,
            gmail_thread_id,
            subject,
      ...
      // Old schema without folders column (fallback)
      threadResult = await client.query<{ id: string }>(
        `INSERT INTO threads (
            tenant_id,
            gmail_thread_id,
  ...
  // Generate AI summary for the thread (only if it doesn't have one yet)
  try {
    const hasSummary = await withTenant(tenantId, async (client) => {
      const { rows: threadRows } = await client.query(
        `SELECT summary FROM threads WHERE id = $1 AND summary IS NOT NULL LIMIT 1`,
        [upsertResult.threadId]
      )
      return threadRows.length > 0
    })

    // Only generate summary if thread doesn't have one
    if (!hasSummary) {
    ...
    console.error(
      `Summary generation failed for thread ${upsertResult.threadId} (tenant ${tenantId}):`,
      error,
    )
  }
}
